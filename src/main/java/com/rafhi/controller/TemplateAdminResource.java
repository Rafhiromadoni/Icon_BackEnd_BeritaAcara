...
public class TemplateAdminResource {
    ...
    public Response updateTemplate(@PathParam("id") Long id, DefineTemplateRequest request) {
        ...
        // 1. Update data utama template
        template.templateName = request.templateName;
        template.description = request.description;
        template.isActive = request.isActive;

        // 2. Hapus placeholder lama untuk diganti dengan yang baru
        // Ini cara paling sederhana untuk sinkronisasi
        template.getPlaceholders().forEach(ph -> ph.delete());
        template.getPlaceholders().clear();

        // 3. Tambahkan placeholder baru dari request
        for (TemplatePlaceholder newPh : request.placeholders) {
            newPh.template = template; // Hubungkan ke template ini
            newPh.persist();
